...
class StoryEvaluator:
    ...
    def evaluate_all_categories(self, story: str) -> dict[str, EvaluationResult]:
        ...
        # Build a single prompt for all categories
        categories_list = "\n".join([f"{i+1}. {cat}" for i, cat in enumerate(STORY_EVALUATION_CATEGORIES)])
        
        combined_prompt = (
            f"Evaluate the following story across all these categories. "
            f"For each category, provide a score from 1.0 to 10.0 (can include one decimal place like 7.5).\n\n"
            f"Categories to evaluate:\n{categories_list}\n\n"
            f"Respond with JSON containing a 'scores' object where each key is the category name and the value is the score (number).\n"
            f"Example format: {{\"scores\": {{\"Grammar, spelling, and punctuation quality\": 8.5, \"Sentence pattern variety\": 7.0, ...}}}}\n\n"
            f"Story:\n{story}"
        )
        
        # Single LLM call for all categories
        response = self._client.chat(
            system_prompt="You are a literary critic. Always respond with valid JSON containing a 'scores' object with category names as keys and numeric scores (1.0-10.0) as values.",
        ...
        try:
            ...
            for category in STORY_EVALUATION_CATEGORIES:
                ...
                results[category] = EvaluationResult(
                    category=category,
                    score=score,
                    explanation="",  # No explanation to save time
                )
        ...
        creativity_prompt = (
            ...
            f"Evaluation Scores:\n{category_summary}\n\n"
            f"Original Story:\n{story}\n\n"
            f"Respond with JSON: {{\"score\": <number>, \"explanation\": \"<short justification>\"}}"
        )
        creativity_response = self._client.chat(
            system_prompt="You are a literary critic. Always respond with valid JSON containing 'score' (number) and 'explanation' (string).",
